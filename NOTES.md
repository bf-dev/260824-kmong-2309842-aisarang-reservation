...
### 매니페스트 백업 (되돌리기 경로) — 손볼 것이 하나 있다

매니페스트 파일은 리포에 없다. gateway artifacts 아래 한 곳에만 있다:
`/home/bfdev/neoworks/apps/gateway/artifacts/public/2309842/version-aisarang.json`

게시할 때마다 직전 판을 떠 두고 있다. 현재 위치:

    ~/workspace/kmong/tmp/version-aisarang.json.1.0.12.bak
    ~/workspace/kmong/tmp/version-aisarang.json.1.0.13.bak

**문제: `tmp/` 는 14일 뒤 청소된다.** 즉 9월 말이면 되돌리기 사본이 조용히
사라진다. ZIP 은 계속 서빙되므로 되돌리기 자체가 불가능해지는 것은 아니고,
버전/URL/sha256/크기는 이 NOTES 의 각 '배포 현황' 절에 적혀 있어 재구성할 수
있다. 다만 `notes` 의 한국어 본문만은 어디에도 안 남아서 그것만 잃는다.

제안(소유자 판단): 리포에 `deploy/manifests/version-aisarang-<ver>.json` 으로
같이 커밋한다. 매니페스트에는 비밀이 없다(버전·URL·sha256·크기·안내문뿐)
이므로 커밋해도 안전하고, 그러면 되돌리기 사본이 버전 관리까지 붙는다.

    mkdir -p deploy/manifests
    cp ~/workspace/kmong/tmp/version-aisarang.json.1.0.1{2,3}.bak deploy/manifests/


**기록 전용 판.** 대기열 순번을 로그와 업로드 요약에 남긴다. 판정/조준/대기/
중복제출 가드는 1.0.13 과 **완전히 동일**하다. 코드 diff 는 기록용 필드 추가와
한 줄의 문구뿐이고, 제어 흐름은 한 군데도 바뀌지 않았다.

왜 로그 한 줄에 판을 쓰는가: 고객이 반복해서 묻는 것이 「우리가 늦었나」 다.
진 날 로그가 「앞에 31명」 이라고 말하면 그 질문에 그날 바로 답이 된다.
대기열은 이길 수 없다(위 조사 참고). 이길 수 없으면 **몇 번째로 졌는지는 남긴다.**

- 프로그램: https://works.insu.ng/works/public/2309842/aisarang-reservation-1.0.14.zip
  (29,290,322 bytes, HTTP 200 확인)
  sha256 `7bed504d7bc7b6407f3a57be579835485a4ab80bb844542c82fd9d21e8868396`
  CI 값 = 내려받은 아티팩트 = Cloudflare 엣지가 주는 바이트, 셋이 일치. `unzip -t` 통과.
- 매니페스트: https://works.insu.ng/works/public/2309842/version-aisarang.json
  `version 1.0.14` / `updatedAt 2026-09-15T02:35:00Z` / `supersedes 1.0.13` /
  `zipUrl` 만 (**exeUrl 없음**: 1.0.4 이하 업데이터가 ZIP 을 exe 자리에 덮어쓰는
  사고를 막기 위해서다. 이 규칙은 바꾸지 말 것). 소유자 지시로 게시했다.
  원자적 기록(임시파일 → `os.replace`), 디스크 사본과 서빙 사본이 **바이트 단위로
  동일**(sha256 `50e431fd8a1569723ce8bab31ebae8f16368af61a38e7e31719c378144e8ed0b`,
  공개 DNS 와 로컬 Caddy 양쪽). 이 JSON 은 `cf-cache-status: DYNAMIC` 이라 엣지가
  캐시하지 않는다.
- 게시 후 실제 업데이터에 **서빙 중인 매니페스트를 그대로** 먹여 확인:
  1.0.9/1.0.11/1.0.12/1.0.13 → 전부 `zip 1.0.14`, 1.0.14 → `None`(재시작 루프 없음).
- 되돌리려면 매니페스트를 1.0.13 으로 돌리면 된다(1.0.13 ZIP 은 계속 서빙된다).
  백업 사본 위치는 아래 '매니페스트 백업' 절 참고.
- CI: GitHub Actions run **34920495857**, `89e4b16` 에서 **success**. 전체 **292 passed**.
  `ci\swap_check.py`: `OK[ascii]` / `OK[korean]` / `SWAP CHECK: OK` (`$LASTEXITCODE` 게이트).
  이날 러너에서는 실사이트가 안 닿아 live selftest 가 skip 되고 픽스처 경로가 돌았다(설계된 대체 경로).
- 프로즌 exe 가 보고하는 판 번호: **v1.0.14** (GUI 제목표시줄/머리말 실제 픽셀).
  PE 버전 리소스 1.0.14.0.
