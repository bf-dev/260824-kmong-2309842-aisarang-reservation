...
## 2026-09-15 사고: 우리가 **거짓 성공**을 찍었다 (v1.0.13 에서 수정)

이 프로젝트에서 가장 나쁜 실패다. 고객은 예약을 못 했는데 우리 프로그램은
'예약 성공' 이라고 말했다. 없는 자리를 있다고 알려준 것이다.

그날 고객 PC 로그(v1.0.12):

    [08:59:59] 조준 확정: 도착 목표 정각 +275ms (시각 오차 ±25ms + 여유 250ms)
    [09:00:00] [확인] 1발째 · 도착 추정 정각 +278ms · 서버: 예약이 완료 [ok · 예약 성공]
    [09:00:00] 판정 근거: 화면 안내 문구 (서버 응답 본문은 못 봤습니다)

고객 원문: 「오늘은 예약을 실패하였는데 이미 선예약 되어있다는 메세지가 떴습니다」
그리고 예약 내역 확인 후: 「예약은 어플에서도 모두 실패하였습니다 먼저 다른
예약이 되어있었습니다」 → **20260929 자리는 제3자가 가져갔고 고객은 예약이 없다.**

### 그때 화면에 있던 것 (verbatim, 진단 ZIP)

`...-2309842-20260915-090001.zip` 의 `page_source/0002_handover_after.html`,
`<div id="NetFunnel_Loading_Popup" style="display: block; ... visibility: visible">`
안, 태그를 걷어낸 원문 그대로:
...
**결과 알림이 아니라 가상대기열 안내다.** 같은 ZIP 에서:

- `layer-alert-popup-contents*` 여섯 개가 **전부 비어 있다**. 결과 알림은 없었다.
- `xhr_bodies_handover_after.json` 의 `submit` 이 `null` (`submitSeen: false`).
  InsertOcreqst 는 **나가지도 않았다**.
- 네트워크 덤프에 `ts.wseq?opcode=5101`(대기열 진입) 한 건뿐이고 09-09/09-14 에
  있던 `opcode=5004`(반납) 가 **없다**. 줄에 선 채로 run 이 끝났다.
- **'선예약' 이라는 글자는 그 ZIP 어디에도 없다.** 고객이 그 문구를 본 것은
  우리가 이미 '성공' 을 찍고 끝낸 뒤다. 서버가 보낸 선예약 원문의 출처는
  지금도 09-04 캡처(`insert_ocreqst_taken.json` = `booking.TAKEN_REAL`) 하나뿐이다.
  두 근거를 섞어서 "09-15 에서 선예약 문구를 읽었다" 고 적지 말 것.

### 결함 1 — 조각 일치가 실패 문장을 성공으로 읽었다

`OK_WORDS` 에 `"예약이 완료"` 라는 **조각**이 있었고, `_scan_page_source` 가
`driver.page_source` **통짜**에서 그 조각을 찾아 `(R_OK, "예약이 완료")` 를
돌려줬다. 로그의 '서버 문구' 가 문장이 아니라 조각이었던 것이 바로 그 증거다.

`_RE_TAKEN` 이 안 터진 이유는 정규식이 틀려서가 아니다. **그 페이지에 선예약
문구가 아예 없었다.** 처음 세웠던 가설("…선예약되어 예약이 완료되지 않았습니다"
같은 부정문을 조각으로 집었다")은 실물과 다르다. 진짜 범인은 넷퍼널 대기열
보일러플레이트다.

그리고 이 독은 2026-08-26 부터 리포 안에 있었다. `ci/fixtures/real/netfunnel_waiting.html`
에 같은 문장이 그대로 들어 있는데, **그 픽스처를 분류기에 물려본 테스트가 없었다.**

고친 방식 (`aisarang/booking.py`):

- `_RE_OK` — 성공은 '예약/신청' 에 붙은 서술어가 **과거 완료 종결**일 때만.
  `되었/됐/하였/했 + 습니다`. `완료됩니다`(미래)도 `완료되지 않았습니다`(부정)도
  매치되지 않는다.
- `_RE_NEGATED` / `_RE_PENDING` — 같은 **문장** 안에 부정이나 대기 표현이 있으면
  완료형이라도 성공이 아니다. 부정은 문장 단위로만 본다(`sentences()`).
  페이지 통짜를 한 덩어리로 보면 옆 문장의 '않' 이 진짜 성공을 지운다.
- `says_ok()` 가 유일한 성공 판정 경로다. `OK_WORDS` 는 남아 있지만 이제
  **분류용이 아니다**(문장을 찾는 실마리일 뿐).
- `_RE_NOT_DONE` — '선예약' 이 안 붙은 `…예약되지 않았` 도 R_FAIL 로 떨어뜨린다.
- `_scan_page_source` 가 (1) 사이트의 진짜 알림 컨테이너
  (`layer-alert-popup-contents*`) 를 먼저 보고 (2) 없을 때만 평문을 **문장 단위**로
  훑는다. 그리고 조각이 아니라 **문장**을 돌려준다. 확인창
  (`layer-confirm-popup-contents*`) 은 일부러 뺐다(결과가 아니라 질문이고 본문에
  '불가'/'초과' 가 들어 있다).
- `automation.read_result` 도 같은 조각 일치를 쓰고 있었다. `_scan_page_source`
  로 넘겼다.

### 결함 2 — 응답 본문 경로는 **망가진 적이 없다**. 우리가 안 기다렸다

훅은 멀쩡했다. 09-15 덤프도 `hookInstalled: true` 고, 09-14 덤프에는
InsertOcreqst 응답 본문이 헤더까지 통째로 들어 있다. 회귀는 **캡처가 아니라
`read_outcome_detail` 의 순서**였다. 화면 문구가 분류되면 제출 응답을 기다리지
않고 즉시 리턴해서, `SUBMIT_WAIT_SECONDS = 9.0` 이 **한 번도 쓰이지 않았다**.

실측 (진단 ZIP, 브라우저 시계 KST):

| 날짜 | 5101 진입 | InsertOcreqst t0 → t1 | 5004 | 우리가 리턴한 시점 | 근거 |
|---|---|---|---|---|---|
| 09-09 | 09:00:00.506 | 00.692 → 01.279 (587ms) | 01.276 | waited 881ms | **submit** |
| 09-14 | 09:00:01.359 | 01.522 → 02.047 (525ms) | 02.043 | waited 699ms | screen |
| 09-15 | 09:00:00.504 | **없음** | **없음** | waited **287ms** | screen |

- 09-14: 제출은 잡혔는데(`submitSeen: true, submitDone: false`) 답을 **77ms 앞두고**
  화면으로 끝냈다. 본문은 그 직후 왔고 ZIP 에 `returnval: success` 로 남아 있다.
- 09-15: 넷퍼널 줄에 서 있어서 제출이 나가지도 않았고(`submitSeen: false`),
  287ms 만에 대기열 안내를 성공으로 읽고 끝냈다. 예상 대기는 3초였다.
  **9초를 기다렸다면 진짜 서버 답(선예약)을 읽었을 것이다.**

고친 규칙 세 줄 (`read_outcome_detail`):

1. 제출이 오는 중이면(`seen && !done`) 화면을 **아예 읽지 않는다.** 본문이 유일한 근거다.
2. 가상대기열이 떠 있으면 아직 아무것도 제출되지 않은 것이다. 화면의 어떤 문구도
   이번 발사의 결과가 아니다. `Outcome.queued` 로 남긴다.
3. 화면만 보고 내리는 **성공** 판정은 화면 창(`timeout`)을 다 쓴 뒤에만 받아들인다.
   성공이 아닌 판정은 예전처럼 즉시 돌려준다(재시도 타이밍을 잃지 않으려고).

### 중복 예약 방지 (같이 들어간 안전장치)

판정이 `unknown` 인데 **제출은 이미 나갔거나 대기열에 걸려 있으면** 다시 쏘지
않고 멈춘다(`handover.burst`, `booking.confirm_burst`, reason `unknown_submitted`).
이 사이트는 취소가 전화로만 되므로 중복 예약은 놓친 예약보다 확실히 나쁘다.
예전에는 unknown 이 곧 `redrive_confirm`(= [예약하기] 재클릭)이었다.

### 09-14 는 믿을 수 있는가 — 결과는 예, 판정 경로는 아니오

- **결과는 믿을 수 있다.** 그날 ZIP 두 개 모두 `xhr_bodies_handover_after.json` 에
  `{"returnmsg":"1건 예약 중 1건 예약되었습니다.","returnval":"success"}` 가 있고
  응답 헤더 `date: Mon, 14 Sep 2026 00:00:00 GMT` 까지 붙어 있다. 서버가 성공이라고
  했다. 화면의 알림 팝업도 실물로 같은 문장을 띄웠다(`layer-alert-popup-contents2`).
- **그러나 그날의 판정 경로는 09-15 와 같은 결함 위에 서 있었다.** 로그에 남은
  '서버 문구' 는 문장이 아니라 조각(`예약되었습니다`)이고, 그건 통짜 page_source
  훑기에서 나온 값이다. 즉 **운이 좋아 맞은 것**이지 근거가 옳아서 맞은 게 아니다.
  오프라인 재현으로 확인했다(아래).
- 요약: 09-14 의 예약은 진짜다. 09-14 로그의 `[ok]` 를 "판정기가 건강하다" 는
  증거로 인용하지 말 것.

### 오프라인 재현 (실물 151KB 페이지, 라이브 제출 없음)

`_scan_page_source` 를 고객 화면 원본에 그대로 물린 결과:

| 캡처 | 실제 결과 | v1.0.12 (서빙 중) | v1.0.13 (수정) |
|---|---|---|---|
| 09-09 | 성공 (본문 success) | `('ok','예약되었습니다')` | `('ok','1건 예약 중 1건 예약되었습니다.')` |
| 09-14 x2 | 성공 (본문 success) | `('ok','예약되었습니다')` | `('ok','1건 예약 중 1건 예약되었습니다.')` |
| **09-15** | **패배 (제3자 선예약)** | **`('ok','예약이 완료')`** | **`None` → unknown** |

그날의 거짓 성공이 그대로 재현되고, 고친 뒤 사라지고, 진짜 성공 셋은 살아 있다.
재현 스크립트는 `~/workspace/kmong/tmp/` (휘발성) 에 있었다. 항구적인 근거는
아래 픽스처와 테스트다.
...
- `ci/fixtures/real/netfunnel_queue_20260915.html` — 그날 화면에서 글자 그대로
  잘라낸 대기열 팝업 + **비어 있는** 알림 컨테이너 세 개. 개인정보 0건
  (`test_the_real_fixtures_carry_no_personal_data` 가 감시한다).
- 전체 스위트 **276 passed**.

### 고객의 "오늘은 프로그램이 좀 늦었던 것 같다" 에 대해 — 로그는 지지하지 않는다

조준 +275ms, 도착 추정 +277.5ms, **오차 2.5ms**. 시계는 그날 세 번 재측정했고
편차 0.0ms, 불확실도 ±50ms, 17분 동안 총 표류 -6ms 였다. 최근 두 번의 **성공**은
+275ms / +277ms 로 사실상 같은 자리다. **우리 클릭은 늘 있던 자리에 떨어졌다.**
바뀐 것은 우리 타이밍이 아니라 환경이다: 09-09/09-14 는 넷퍼널이 즉시 통과시켰고
(5101 → 5004), 09-15 는 앞에 31명을 세웠다.

**조준을 앞으로 당기자는 요청은 받지 말 것.** 2026-08-27 실측에서 정각 -296ms
도착은 「아직 예약 가능한 시간이 아닙니다」 로 서버가 버렸다. 당기면 잃은 자리가
**확정적으로** 잃은 자리가 된다. (그 -296ms 측정은 09-15 캡처로는 확인도 반박도
불가능하다. 그날 정각 전에 제출한 것이 없어서 해당 문구가 네 ZIP 어디에도 없다.)

### 09-15 캡처가 답하지 **못하는** 것 (추측으로 메우지 말 것)

- **정각이 앞당겨 열렸는가.** 답 못 한다. 그 ZIP 에는 정각 이후의 예약 제출도,
  서버의 예약 응답도, 자리 상태 payload 도 없다. 정각 뒤 유일한 증거는 대기열
  진입 한 건(브라우저 09:00:00.504 = 측정 오프셋 -240.8ms 로 환산해 서버
  약 09:00:00.26)뿐이고, 그건 **대기열 혼잡도**지 자리 상태가 아니다.
  "앞에 31명" 도 정각 전에 누른 사람이 많았다는 뜻일 뿐이다(고객 본인이
  59분 58초에 어플로 눌렀을 때 대기화면을 봤다고 했다 = 넷퍼널은 정각 전에도
  번호표를 준다).
- **누가 자리를 가져갔는가.** 답 못 한다. 진단 캡처는 **우리 브라우저의 트래픽만**
  본다(CDP + 페이지 훅). 고객 휴대폰 앱의 요청은 물리적으로 보이지 않는다.
  자리 소유를 가리는 값도 없다: 09-29 표는 시간마다 잔여 수(그날 2)만 내려주고
  예약 ID 나 소유자 플래그가 없다. 유일한 계정 단위 근거는 셋업 때(약 08:43)
  `SelectDupleTime` 가 돌려준 `{"returnValue":"N"}`(그 시점엔 중복 없음)뿐이다.
  이 질문은 고객의 **예약 내역 화면**만 답할 수 있고, 실제로 그렇게 답이 나왔다.

## 진단 업로드는 언제 일어나는가 (그리고 "업로드 없음" 이 뜻하는 것)

2026-09-15 측정. **"고객 PC 에서 업로드가 없다" 는 아무것도 뜻하지 않는다.**
이것을 실패 신호로 읽다가 이 프로젝트에서 이미 여러 번 헛짚었다.
...
| 어디 | 언제 |
|---|---|
| `runner.py:788` | **예약 실행이 끝날 때마다** (성공/실패 무관). `mode: live`. 09-15 에는 09:00:01 |
| `main.py:612` | **창을 닫아 프로그램이 끝날 때.** `mode: gui, result: closed`, `blocking=True` |
| `reporter.py:261` (`upload_exception`) | **처리 못 한 예외.** 호출부: `gui.py:45`(모든 버튼 핸들러를 감싸는 safe_handler), `runner.py:136`, `main.py:617`, 그리고 `install_excepthook` |
| `recorder.py:519` | 진단 기록 모드를 멈출 때 |

우리 전용 모드(고객 경로 아님): `main.py` 215 selftest / 231 arrivaltest /
270 clocktest / 570 handovertest.

**시작할 때는 업로드가 없다. 자동 업데이트가 성공해도 업로드가 없다.**
그래서 고객이 프로그램을 켜 두고 그냥 두면 09시 실행이 끝나거나 창을 닫을
때까지 몇 시간이고 업로드가 0건이다. 그 침묵은 업데이트 실패의 증거가 아니다.

### 그래도 확인할 방법은 있다 — Caddy 접근 로그

업데이터는 **우리 서버**를 두드린다. 그래서 고객 PC 가 업로드를 하지 않아도
`works.insu.ng` 접근 로그에 흔적이 남는다.

    /var/log/caddy/works.insu.ng.access.log   (JSON 한 줄씩, sudo 필요)

고객 PC 식별자: `X-Forwarded-For 58.141.245.130`, `Cf-Ipcountry: KR`,
`User-Agent: python-requests/2.32.3`.
추측이 아니라 고정점이 있다: 08:41:57 의 조회가 09-15 진단 ZIP 의
`startedAt 2026-09-14T23:41:55Z`(= 08:41:55 KST)보다 2초 뒤다. 같은 세션이다.

`UpdaterThread.run()` 은 **먼저 한 번 조회하고** 그 다음부터
`CHECK_SECONDS = 900` 간격으로 돈다(`updater.py:42,168`). 그래서 900초가
아닌 간격의 조회는 **새 프로세스가 떴다는 뜻**이다.

2026-09-15 고객 PC 의 실제 기록:

    08:41:57  version-aisarang.json              200    977B  (1.0.12 매니페스트의 gzip)
    08:56:57  version-aisarang.json              200    977B  (정확히 900초 뒤 = 정상 주기)
    11:35:34  version-aisarang.json              200    860B  (1.0.14 매니페스트의 gzip, 실측 대조함)
    11:35:36  aisarang-reservation-1.0.14.zip    200  29,290,322B   ← 전량 내려받음
    11:35:45  version-aisarang.json              200    860B  (11초 뒤 = 주기가 아님)

읽는 법:

1. 11:35:36 의 ZIP 전량(200, content-length 일치) = 업데이터가 돌았고 1.0.14 를
   봤고 받았다. 동시에 **그 시점의 판이 1.0.14 미만**이었다는 뜻이기도 하다
   (1.0.14 였다면 `choose_download` 가 `None` 을 돌려줘 아무것도 안 받는다).
2. 11:35:45 의 조회는 직전 조회에서 11초 뒤다. 900초 주기일 수 없다.
   = **교체 후 재실행된 새 프로세스의 첫 조회.**
3. 그 뒤로 ZIP 재다운로드가 없다. 1.0.13 이 돌고 있었다면 또 받았을 것이다.
   = 지금 도는 판이 `None` 을 받았다 = **1.0.14 다.**

즉 교체는 11:35:45 KST 경에 끝났고 고객은 1.0.13 을 건너뛰고 한 번만 받았다.

**그리고 그 추론은 그날 바로 확정되었다.** 11:41:51 / 11:42:11 에 고객 PC 가
업로드했고(창을 닫아서), 그 meta 가 이렇게 말한다:

    os          Windows 10 (10.0.19045)        ← 고객 PC
    appVersion  1.0.14
    startedAt   2026-09-15T02:35:44Z           ← 11:35:44 KST
    updateState swapLog: swap start pid=20216 ... robocopy=1  relaunched=1
    run.log     [11:35:44] aisarang-reservation v1.0.14 시작

로그 흔적만으로 추정했던 재실행 시각(11:35:45)과 실제 프로세스 시작 시각
(11:35:44)이 1초 차이다. 즉 **Caddy 접근 로그만으로도 교체 완료를 정확히
읽어낼 수 있다**는 것이 이 건으로 검증됐다. 고객은 1.0.13 을 건너뛰고
1.0.14 하나만 받았다.

함정 하나: `installDir` 은 여전히 `...aisarang-reservation-1.0.11` 이다.
업데이터는 폴더 안의 파일만 바꾸고 폴더 이름은 그대로 두기 때문이다.
**설치 폴더 이름으로 판 번호를 읽지 말 것.** `appVersion` 을 봐라.

## 가상대기열(넷퍼널) 조사 — 2026-09-15, 오프라인 분석

고객 질문: 「내일도 오늘처럼 예약이 몰린다면 같은 결과가 나올수도있겠지요?」
1.0.13 은 결과를 **읽는** 법을 고쳤지 **먼저 들어가는** 법을 고친 게 아니다.
그래서 대기열이 이제 승부의 전부고, 우리는 그것을 한 번도 조사한 적이 없었다.

**표본.** 진단 ZIP 234개 중 고객 PC(`Windows 10 (10.0.19045)`)는 **43개**.
나머지는 우리 CI(`Windows 2025Server (10.0.26100)` 162개, Linux 29개)이고
`installDir` 이 `D:\a\` 로 시작한다. 43개 중 09시 실전(live)은 12일치다.
CI 업로드를 고객 실행으로 잘못 읽는 사고가 실제로 있었으니 **반드시 os 로 거를 것.**

### 티켓은 [예약하기] 에서 끊긴다 (페이지 로드도, [확인] 도 아니다)

`ts.wseq?opcode=5101`(대기열 진입) 시각과 우리 로그의 '확인창을 감지했습니다'
시각이 **초 단위로 일치**한다. 즉 5101 은 사람이 [예약하기] 를 누른 순간 나가고,
확인창은 통과한 뒤에 열린다.

| 날짜 | [예약하기](확인창 감지) | 5101 | 통과(5004) | 결과 |
|---|---|---|---|---|
| 08-27 | 08:39:49 | 08:39:49.187 | 09:00:04 | fail(조준 -296ms) |
| 09-01 | 08:42:20 | 08:42:19.841 | 09:00:09 | fail(선예약) |
| 09-02 | 08:44:02 | 08:44:01.887 | 09:00:06 | fail |
| 09-04 | 08:44:43 | 08:44:42.696 | 없음 | **fail(선예약)** |
| 09-07 | 08:56:21 | 08:56:20.520 | 없음 | win |
| 09-08 | 08:49:17 | **09:00:01.241** | 09:00:02.011 (770ms) | win |
| 09-09 | 08:39:15 | **09:00:00.506** | 09:00:01.276 (770ms) | win |
| 09-14 | 08:43:44 | **09:00:01.359** | 09:00:02.043 (684ms) | win |
| 09-15 | 08:43:48 | **09:00:00.504** | **끝내 없음** | **loss(앞에 31명)** |

두 부류다.

- **티켓을 미리 쥔 날**(08-27·09-01·09-02·09-04·09-07): [예약하기] 에서 5101 이
  나가고 `sessionStorage.NetFunnel_ID = "5002:200:key=..."` 가 정각까지 남아 있다.
  정각의 [확인] 은 그 표를 **재사용**하므로 새 5101 이 없다.
- **정각에 티켓을 끊은 날**(09-08·09-09·09-14·09-15): [예약하기] 때 넷퍼널
  트래픽이 **아예 없고**(sessionStorage 에도 없음), 우리 [확인] 클릭이 5101 을
  만든다. 09-08/09/14 는 684~770ms 만에 통과했고, **09-15 만 앞에 31명**을 만났다.

무엇이 두 부류를 가르는가: **사이트가 그날 그 순간 게이트를 켜뒀는지**다.
우리 버전도, 우리 조준도, 누른 시각도 아니다. 09-04 는 08:44:43 에 눌러 표를
받았고 09-14 는 08:43:44 에 눌러 못 받았다. 1분 차이에 정반대다.
캡처 창이 짧아서 놓친 게 아니다: 네 날 모두 [예약하기] 시점보다 **앞선**
childcare 요청이 기록되어 있다(09-15 는 33초 앞, 09-09 는 20초 앞).

### 대기열은 정각 전에 이미 형성된다

2026-08-26 v1.0.7 캡처(`netfunnel_waiting.html` 의 출처)가 직접 증거다.
[예약하기] 를 누를 때마다:

    08:56:19 → 앞에 72명,  예상 2분 10초
    08:56:56 → 앞에 138명, 예상 3분 50초
    08:57:32 → 앞에 177명, 예상 4분 32초

정각 3~4분 전에 이미 72명이 서 있었고, 확인창은 순번이 올 때까지 열리지 않았다.
고객이 08:59:58 에 어플에서 대기화면을 본 것과 일치한다. 09-15 에는 정각
+0.26초(서버 기준)에 앞에 31명 · 예상 3초였다.

### 그래서 티켓을 일찍 받으면 유리한가 — **아니다. 반례가 있다.**

**09-04.** 08:44:42 에 표를 받아 정각까지 쥐고 있었고, 정각에 대기열이 전혀
없었으며, 제출이 09:00:01.289 에 나갔다. 서버 답:
`1건 예약 중 1건 예약이 선예약으로 인해 예약되지 않았습니다.` (09:00:04.707 도착)
**완벽한 조기 티켓으로 게이트를 그냥 통과하고도 자리를 뺏겼다.**

표는 **문을 통과시켜 줄 뿐, 내 INSERT 를 남의 INSERT 보다 앞세워 주지 않는다.**
그리고 일찍 받으러 가는 것 자체가 위험하다: 08-26 이 바로 그 경우로,
줄이 이미 서던 시간대에 [예약하기] 를 눌러 72 → 138 → 177 로 밀리고 그날을 통째로 잃었다.

### 결론: 정각 이후의 순번은 운이다

09-08·09-09·09-14·09-15 는 **같은 코드, 같은 방식, 0.9초 이내의 같은 순간**에
표를 끊었다. 셋은 앞에 0명이었고 하나는 31명이었다. 그 차이를 만든 것은
**그 1초에 같이 누른 다른 사람의 수**이지 우리가 한 어떤 것도 아니다.

**하지 말 것(둘 다 측정으로 배제됨):**
- [예약하기] 재클릭으로 대기열 재진입 → 08-26, 72명 → 177명, 그날 손실.
- 조준을 정각 앞으로 → 08-27, -296ms 도착은 서버가 그냥 버린다.

고객이 지금 하는 것(몇 분 전에 [예약하기] 를 눌러 확인창을 띄워 두기)이 이미
최선이다. 바꿀 것이 없다. 없는 대책을 지어내지 말 것.

### 눈은 뜰 수 있다 (순번을 **기록**하는 것)

순번 데이터는 이미 우리 손에 있다. `_JS_QUEUE` / `queue_info()` 가
`NetFunnel_Loading_Popup_Count`(앞), `_NextCnt`(뒤), `_TimeLeft`(예상),
`Progress_Print` 를 읽는다. 실제로 채워진 적도 있다: 09-01 meta 에
`queue=True ahead=256 behind=53 eta='39초'`.

그런데 **정작 중요한 자리에서 버리고 있다.** v1.0.13 의 발사 후 대기 루프
(`booking.py` 의 `read_outcome_detail`)는 `queue_info()` 를 부르고 **불린 하나만**
쓴다. `evidence_line` 도 "가상대기열에 선 채로 N초" 라고만 하고 숫자가 없다.
`queue_line(q)` 라는 포맷터가 이미 있으므로 한 줄이면 된다.
그러면 다음에 진 날 고객에게 「오늘은 앞에 31명이었습니다」 라고 말할 수 있다.
...
- 프로그램: https://works.insu.ng/works/public/2309842/aisarang-reservation-1.0.15.zip
  (29,297,014 bytes, HTTP 200 확인)
  sha256 `5dce7ca3994711bc85045c6315bda77a29f5610f8b246370751fd76c143297c5`
...
- 매니페스트: https://works.insu.ng/works/public/2309842/version-aisarang.json
  `version 1.0.15` / `updatedAt 2026-09-17T02:00:00Z` / `supersedes 1.0.14` /
  `zipUrl` 만 (**exeUrl 없음**. 이 규칙은 바꾸지 말 것: 1.0.4 이하 업데이터가
  ZIP 을 exe 자리에 덮어쓴다). 원자적으로 썼다(임시파일 → `os.replace`, 0644).
  사본을 `deploy/manifests/version-aisarang-1.0.15.json` 에 커밋했다.
- 게시 후 **서빙 중인 매니페스트를 실제 업데이터 로직에 그대로 먹여** 확인:
  1.0.9 / 1.0.11 / 1.0.12 / 1.0.13 / **1.0.14 → zip 1.0.15**,
  1.0.15 → `None`(재시작 루프 없음).
- CI: GitHub Actions run **35171421016**, `86cac45` 에서 **success**.
  windows-latest, 전체 스위트 **302 passed**. Defender onedir/zip 둘 다 CLEAN.
  프로즌 exe 가 스스로 보고하는 판 번호 **v1.0.15**(버전 리소스 1.0.15.0,
  세션 1 스크린샷의 제목표시줄).
  `swap_check`: `OK[ascii]` / `OK[korean]` / `SWAP CHECK: OK`.
- 전달 경로: **자동 업데이트**. 고객은 1.0.14 를 돌리고 있고, 프로그램을 켜면
  다음 실행에서 1.0.15 로 바뀐다. 손으로 설치할 것이 없다.
- 되돌리기: 1.0.14 ZIP 은 계속 서빙된다.
  `deploy/manifests/version-aisarang-1.0.14.json` 을 그 경로에
  `install -m 0644` 로 다시 쓰면 끝이다.

### CI 가 이번 판에서 두 번 섰다. 둘 다 하네스 문제였고 제품은 멀쩡했다

다음 사람이 같은 자리에서 헤매지 않도록 남긴다. 둘 다 새로 넣은
`ci/too_early_retry_check.py` 가 원인이고, 제품 코드는 두 번 모두 초록이었다
(단위 시험 / 실물 마크업 / 인계 모드 전부 통과한 뒤 이 단계에서만 섰다).

1. **run 35170310838 — `UnicodeEncodeError: 'charmap' codec`**
   이 하네스는 `handover.burst` 의 **한국어 로그 줄**을 그대로 찍는데,
   windows-latest 에서 이 스크립트의 stdout 은 cp1252 다. 첫 한글 줄에서
   시험 자체가 죽었다. `main.py` 가 이미 겪고 고친 그 문제라 같은 처리를
   했다(`_out`: 못 찍는 글자만 `replace` 로 바꾸고 줄은 남긴다). 판정에 쓰는
   `RETRY...` 줄은 전부 ASCII 라 영향이 없다.
   재현/검증: `PYTHONIOENCODING=cp1252 python ci/too_early_retry_check.py` → exit 0.
2. **run 35170849627 — `early-repeat` 가 7발 전부 `too_early`, `exhausted`**
   **이것은 제품이 옳게 멈춘 것이다.** 한 회복 주기(발사 → 제출 응답 →
   알림 닫기 → [예약하기] → 확인창)가 이 서버에서는 약 350~400ms 인데
   러너에서는 약 **165ms** 였다. 그래서 내가 잡은 서버 지연 1.2초는 러너에서
   7주기를 요구했고 상한 6회에 걸렸다. 상한이 정확히 그 일을 하는 것이라
   제품이 아니라 시나리오를 줄였다(지연 0.25s / 0.6s → 주기가 165ms 든
   400ms 든 6회 안에 끝난다).
   **교훈: 이 하네스에 벽시계 기반 수치를 넣을 때는 러너가 이 서버보다
   2배 이상 빠르다고 보고 잡을 것.** 발사 횟수를 단정하지 않고 불변식만
   단정하는 이유도 이것이다.

### 왜 (09-17 09:00:00 고객 PC 실측)

```
조준 확정: 도착 목표 정각 +275ms (시각 오차 ±25ms + 여유 250ms)
[확인] 1발째 · 도착 추정 정각 +279ms · 서버: 1건 예약 중 1건 예약이
       선예약으로 인해 예약되지 않았습니다. [taken]
판정 근거: 서버 응답 본문 (HTTP 200, 왕복 306ms)
serverOffsetMs -2011.3 / oneWayMs 25.5 / queueAhead 29
```

그날 고객은 휴대폰 앱으로 **손으로** 우리보다 먼저 넣었다. 자리가 279ms 안에
나간다는 뜻이고, 그 279ms 중 250ms 는 우리가 스스로 붙인 여유였다.

공식은 그대로다: `조준 = 시각 오차(한쪽) + 여유`. 여유만 한 걸음 옮겼다.
같은 조건(±25ms)에서 **+275 → +200ms**.

| 시각 오차 | v1.0.14 조준 | v1.0.15 조준 |
|---|---|---|
| ±24ms  | 274ms | 199ms |
| ±25ms  | 275ms | **200ms** |
| ±100ms | 350ms | 275ms |
...
**왜 한 번에 더 안 깎았나.** 이른 쪽은 서버가 확정 거절하고(08-27 실측 1/1),
늦는 쪽은 자리를 잃는다(09-16 / 09-17). 양쪽 다 비용이 있는 구간이라 크게
옮기면 무엇이 좋아졌는지 알 수 없다. 75ms 씩 옮기고 실측 도착값을 본다.
다음 걸음은 `config.ARRIVAL_SAFETY_MS` 한 줄이다.

하한도 175 로 같이 내렸다. **하한이 여유보다 크면 하한이 조준점을 혼자
정한다** (v1.0.12 에서 이미 그랬다). 둘은 항상 같이 움직일 것.

### 다음 사람 필독 — 상수만 내리면 고객 PC 는 안 바뀐다

이번 판에서 제일 조용한 함정이었고, 실제로 확인했다.

고객 PC 의 `settings.json` 에는 v1.0.14 가 쓴 값이 그대로 박혀 있다:

```
arrival_safety_ms: 250,  reopen_max: 2,  reopen_seconds: 15
```

`save_settings` 가 `DEFAULT_SETTINGS` 의 **모든** 키를 파일에 쓰기 때문이다.
고객이 화면에서 이 값을 바꾼 적은 한 번도 없다(`gui._collect` 는 이 키들을
건드리지 않는다). 그런데 옛 runner 는
`settings.get("arrival_safety_ms", 상수)` 로 읽었다. 즉 상수를 175 로 내려도
**파일의 250 이 이겨서 조준이 275ms 에 그대로 머문다.**

실측 확인: 파일 250 + 새 상수 175 → 조준 275ms.

그래서 세 키를 `config._OBSOLETE` 에 넣어 **죽은 키**로 만들었다(옛
`arrival_lead_ms` 와 똑같은 처리다). `DEFAULT_SETTINGS` 에서도 빼서 이제
파일에 쓰지도 않고 읽지도 않는다. 읽는 쪽은 전부 config 상수로 폴백한다.
`tests/test_arrival.py::test_the_saved_safety_margin_can_no_longer_shadow_the_constant`
가 이 회귀를 막는다. **앞으로 조준 설정의 이름을 바꾸거나 없앨 때도 같은
처리를 할 것.**

### 회복이 이 판의 안전망이다 (여유를 깎았으므로)

```
상한  2회  → config.REOPEN_EARLY_MAX = 6
마감  15초 → config.REOPEN_EARLY_SECONDS = 2.0
```

마감을 **줄인** 것이 이상해 보이지만 방향이 맞다. 15초는 근거가 없던 값이고,
정각 +2초를 지나서도 '예약시간전' 이 온다면 우리가 이른 것이 아니라 서버 시각
추정이 통째로 틀린 것이다. 그때 [예약하기] 를 더 누르면 대기열 순번만 뒤로
밀린다(08-26: 72 → 138 → 177명). 진짜 '우리가 이르기만 했다' 는 정각 직후
수백 ms 안에서만 일어나고, 거기서는 창을 다 쓸 만큼 쏴야 한다(한 회복 주기
실측 400~800ms → 2초에 최대 6회). 상한은 올라갔지만 창이 좁아져서 총
재클릭 가능 시간은 오히려 줄었다.

그리고 **이른 발마다 도착 추정을 다시 배운다.** v1.0.14 는 `corrected`
플래그로 첫 '예약시간전' 하나만 보정하고 그 뒤를 버렸다. 한 발 보정하고도
여전히 이르면 두 번째 응답이 바로 그 사실을 말해주는데 그것을 흘린 것이다.
`note_too_early` 는 단조 증가라(배운 것보다 작으면 0) 매번 불러도 조준이
앞으로 가지 않는다. `handover.burst` 와 `booking.confirm_burst` 둘 다 고쳤다.

### 새 회귀 시험 — `ci/too_early_retry_check.py` (CI 필수 단계)

조준을 당길 근거가 이 파일이다. 실물 캡처 + 진짜 크롬 + **서버 원문**으로
한 브라우저 안에서 회복 전체를 걷는다:

```
[확인] → '아직 예약 가능한 시간이 아닙니다.' (returnmsg 실물)
      → 알림 닫기 → [예약하기] 재클릭 → 확인창 되살아남 → 재발사 → 성공
```

두 시나리오를 돈다. '한 번만 회복한다' 는 구현은 두 번째에서 떨어진다.

```
early-once   codes=['too_early','ok']                reopenUsed=1  learned=[180.1]
early-repeat codes=['too_early' x4,'ok']             reopenUsed=4
             learned=[180.6, 356.9, 309.9, 388.3]    correctionMs=1235.7
```

단정하는 불변식: 마지막 발만 종결 코드 / 이른 발은 전부 `too_early` /
되살리기 횟수 == 이른 발 수 / **제출 수 == 발사 수**(중복 예약 없음) /
모든 판정의 근거가 `source=submit` / 이른 발마다 보정 1회.

발사 횟수 자체는 벽시계(응답 왕복)에 달려 있어 단정하지 않는다. 3회 연속
돌려 전부 통과했다.

시계만 가짜다(09:00 을 기다릴 수 없다). 페이지·마크업·서버 응답 본문·분류기·
회복 경로·재발사는 전부 출하되는 그 코드다. `server_open` 을 우리가 믿는
정각보다 늦게 두는 것이 '예약시간전' 을 여러 번 맞는 유일한 길이다(되살리기
게이트는 우리가 믿는 정각 이후에만 열리므로).

### 판정 규칙은 한 글자도 안 건드렸다

`says_ok` / `_RE_OK` / `OK_WORDS` / `classify` / 키워드 집합 전부 그대로다.
대기열 문구에 대한 조각 일치(v1.0.13 에서 고친 그 버그)는 되살리지 않았다.
`git diff aisarang/booking.py` 의 변경은 `corrected` 플래그 제거 한 곳뿐이다.

### 회귀 시험 결과

- 전체 스위트 **302 passed** (로컬, 크롬 포함)
- `ci/selector_audit.py`: 제품 동작 검증 **26/26**, exit 0
- `main.py --handovertest`: `fired=1/6 expected=1`,
  `tooEarlyText=too_early reopenAllowed=True fnSave=1 alertClosed=1 confirmClicked=0`,
  `takenText=taken reopenAllowed=False fnSave=0`,
  `submitCode=taken submitSource=submit verbatim=True`
- 진단 업로드: `status=200 matched=true` (v1.0.15, 고객 2309842)

## 배포 현황 (v1.0.14, 2026-09-15 02:35Z) ← 지난 판

### 매니페스트 사본은 리포에 둔다 (`deploy/manifests/`)

매니페스트 파일 자체는 리포에 없다. gateway artifacts 아래 한 곳에만 있다:
`/home/bfdev/neoworks/apps/gateway/artifacts/public/2309842/version-aisarang.json`

**게시할 때마다 그 판의 사본을 `deploy/manifests/version-aisarang-<ver>.json`
으로 같이 커밋한다.** 1.0.12 / 1.0.13 / 1.0.14 는 이미 넣어 뒀다.

왜. 예전에는 `~/workspace/kmong/tmp/` 에만 떠 뒀는데 tmp 는 14일 뒤 청소된다.
ZIP 은 계속 서빙되므로 되돌리기가 불가능해지지는 않고, 버전/URL/sha256/크기는
이 NOTES 의 '배포 현황' 절에 있어 재구성할 수 있다. 다만 `notes` 의 한국어
본문(= **그 판을 왜 냈는지**의 기록, 몇 달 뒤에 읽을 사람을 위해 쓴 것)만은
어디에도 안 남아 사라진다. 에러 하나 없이 조용히 없어지는 손실이라 옮겼다.
매니페스트에는 비밀이 없다(버전·URL·sha256·크기·안내문뿐).

기억에 의존하지 않도록 `tests/test_manifest_archive.py` 가 지킨다: 파일명과
`version` 일치, JSON 유효, **exeUrl 없음**(사본에 섞이면 되돌리는 순간 1.0.4
이하 업데이터가 ZIP 을 exe 자리에 덮어쓴다), `notes` 비어 있지 않음,
`zipUrl` 에 같은 판 번호가 박혀 있음.

### 무엇이 바뀌었나 (한 줄 요약)

`read_outcome_detail` 이 `queue_info()` 결과를 **통째로** 들고 있고
(`Outcome.queue`), `as_dict` 가 `queueAhead/queueBehind/queueEta/queueProgress`
를 내보내며, `evidence_line` 이 이미 있던 `queue_line()` 으로 그 숫자를 찍는다.
v1.0.13 까지는 `queue_info()` 를 부르고 **불린 하나만** 남긴 뒤 숫자를 버렸다.
그래서 09-15 에 화면에 떠 있던 '31명' 이 로그에 한 번도 나오지 않았다.

09-15 실물 캡처(138,954 bytes)를 이 판으로 재생한 결과:

    판정 : unknown (거짓 성공 아님)
    근거 : 판정 근거: 없음. 가상대기열 대기 중 (앞에 31명, 뒤에 1명, 예상 03초)
           인 채로 0.9초가 지났습니다 (예약 제출이 아직 나가지 않았습니다).
    요약 : queueAhead=31 queueBehind=1 queueEta=03초 queueProgress=0 % (0/31)

내일 지는 날이 있다면 로그가 가져야 할 모양이 이것이다.

### 회귀 시험 (숫자는 전부 실물)

- `ahead=256 behind=53 eta=39초` — 2026-09-01 meta 의 실제 값.
- `31 / 1 / 03초 / 0 % (0/31)` — 2026-09-15 넷퍼널 팝업의 실제 값.
- 수정 전(1.0.13) 빨간불 먼저 확인: 새 시험 6개가 `Outcome.queue` 없음과
  `queueAhead` 키 없음으로 실패한다.
- 숫자가 아직 없으면 `가상대기열 대기 중` 으로 괄호째 빠진다.
  `None` 도 `앞에 0명` 도 찍히지 않는 것을 단언한다.
- 대기열이 아예 없으면 근거 줄이 1.0.13 과 **글자 그대로 동일**하고
  `as_dict` 에 queue 키가 아예 없다(무행동 변경의 증거).

## 배포 현황 (v1.0.13, 2026-09-15 01:40Z) ← 지난 판
...
- 2026-09-15 02:35Z 에 **1.0.14 로 교체되었다**(위 절 참고). 이 ZIP 은 계속
  서빙되지만 매니페스트는 더 이상 이것을 가리키지 않는다. 09-15 거짓 성공
  수정은 이 판에서 들어갔고 1.0.14 에도 그대로 있다.
...
### 조준값에 대해 (건드리지 말 것)  ← **v1.0.15 에서 뒤집혔다. 아래를 볼 것**
...
> **2026-09-17 정정.** 위 문단은 그 시점(09-02)에는 맞았지만 지금은 근거가
> 달라졌다. 그때 비교한 구간은 +686~+803ms 였고, 그 안에서 도착 시각이
> 결과를 예측하지 못한다는 것은 **여전히 사실이다**. 하지만 그 뒤로 조준점이
> +275ms 까지 내려왔고, 09-16 / 09-17 두 아침은 그 +279ms 에서도 '선예약'
> (= 남이 먼저 가져감)이었다. 09-17 에는 고객이 휴대폰 앱으로 손으로 우리보다
> 먼저 넣었다. 즉 자리가 279ms 안에 나간다는 실측이 생겼고, 그 279ms 중
> 250ms 는 우리가 스스로 붙인 여유였다.
> 그래서 v1.0.15 에서 여유를 175ms 로 한 걸음 줄였다(→ 같은 조건 +200ms).
> 지금 유효한 규칙은 "당기지 말 것" 이 아니라 **"한 걸음씩만 당기고, 이른
> 쪽 회복이 단단할 때만 당길 것"** 이다. 이른 쪽은 서버가 확정 거절하므로
> (08-27 실측 1/1) 회복 없이 당기는 것은 여전히 금지다.
> 맨 위 '배포 현황 (v1.0.15)' 절과 `config.py` 의 ARRIVAL_* 주석을 볼 것.
