...
# ------------------------------------------------------------------ 정각 전 조준 (v1.0.18, 하루 실험)
#
# **이 판은 고객이 2026-09-23 에 직접 요구한 하루짜리 실험이다.** 정각
# **전** 500ms 에 [확인] 요청이 도착하도록 쏜다. 근거는 고객이 그날
# 가져온 실측 네 가지다:
#
#   1) 고객의 휴대폰 앱은 08:59:59.x 에 요청을 날려 성공했다. 대기 화면
#      없이 바로 완료였다.
#   2) 폰 세 대 실험: 정각 전에 누른 첫 요청이 이겼고, 나중에 누른 요청은
#      전부 대기자순번(넷퍼널) 화면으로 갔다.
#   3) 앱은 예전엔 정각 전 요청을 「아직 예약 가능한 시간이 아닙니다」 로
#      거절했는데, **최근에 그 문구 대신 대기열 화면으로 넘겨주기
#      시작했다.** 즉 서버가 정각 전 요청을 무조건 버리는 옛 동작이
#      바뀌었을 가능성이 생겼다.
#   4) 우리 09-23 웹 캡처도 정각 +169ms 에 닿아 즉시 [선예약·마감] 을
#      읽었다. 자리는 그보다 훨씬 일찍 결정났다는 뜻이다.
#
# 실험의 안전망은 회복이다. 발사한 요청이 서버 응답 본문의 진짜 문구
# 「아직 예약 가능한 시간이 아닙니다.」 로 too_early 로 판정되면, 그
# 자리는 아무도 못 가져갔다는 뜻이므로 표준 조준(ARRIVAL_SAFETY_MS,
# 정각 +140ms)으로 다시 누른다(_Reopen). 이 조준은 정각 이후 회복 발사가
# 쓰는 값이기도 하다.
#
# **이 실험은 대기열 화면을 too_early 로 읽지 않는다.** 「대기자순번」 /
# 넷퍼널 로딩이 뜨면 두 번 누르지 않고 기다린다. 서버에 이중 제출
# 방어가 없어서(2026-09-15 / 09-18 실측) 두 번 누르면 우리 발이 우리
# 예약을 깬다. 대기열은 pending 이고, 최종 판정은 InsertOcreqst 응답
# 본문으로만 한다.
...
DEFAULT_SETTINGS = {
    ...
    "setup_seconds": 240,
    # v1.0.18: arrival_after_ms 는 설정에서 죽었다. 조준은 상수
    # (CONFIRM_PREHOUR_LEAD_MS) 로만 정한다. 아래 _OBSOLETE 참고.
...
# v1.0.18 에서 넷째가 죽었다: arrival_after_ms. 정각 **앞** 쪽으로 조준을
# 옮기는 하루 실험(CONFIRM_PREHOUR_LEAD_MS)인데, 만약 옛 settings.json 에
# arrival_after_ms 값이 남아 있다면 runner 가 그것을 읽어 -500ms 을 덮어
# 써버린다. 09-17 에 arrival_safety_ms: 250 이 여유를 그림자처럼 덮어써서
# 조준이 안 바뀌었던 것과 같은 함정이다. 그래서 이 키도 죽은 키로 만든다:
# 파일에서 지우고(load_settings 가 재작성), 읽는 곳도 없다.
_OBSOLETE = ("prefire_ms", "arrival_lead_ms", "arrival_after_ms",
