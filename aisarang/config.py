...
# v1.0.18 에서 넷째가 죽었다: arrival_after_ms. 옛 settings.json 에 이 값이
# 남아 있으면 runner 가 그것을 읽어 새 조준을 덮어써 버린다. 09-17 에
# arrival_safety_ms: 250 이 여유를 그림자처럼 덮어써서 조준이 안 바뀌었던
...
# "파일에 남은 옛 키는 언젠가 누군가 다시 읽는다" 이므로, 되돌릴 때
# 고객 PC 의 파일에서 물리적으로 지워 두는 것이 맞다. 이 판을 켜는 순간
# load_settings 가 지우고 파일을 다시 쓴다.
_OBSOLETE = ("prefire_ms", "arrival_lead_ms", "arrival_after_ms",
             "arrival_safety_ms", "reopen_max", "reopen_seconds",
             "confirm_prehour_lead_ms", "arrival_prehour_lead_ms")
