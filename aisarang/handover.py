...
def burst(driver, clock, open_epoch: float, watcher: Watcher,
    ...
    v1.0.19: 첫 발은 표준 조준(정각 뒤, ARRIVAL_SAFETY_MS 여유)에 도착한다.
    정각 전으로 쏘는 경로는 없다. 그래도 서버가 '예약시간전' 을 돌려주면
    (조준이 빗나갔거나 서버 시각이 앞선 경우) 이 회복 경로가 받아낸다.
      - '예약시간전' 을 맞을 때마다 **매번** 도착 추정을 다시 보정한다.
        `note_too_early` 는 단조 증가라(배운 것보다 작은 값은 무시) 음수
        추정은 0 을 돌려주고 아무것도 배우지 않는다.
