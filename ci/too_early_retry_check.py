...
def main() -> int:
    ...
    #   early-once   이른 발 → 되살리기 → 성공         (브리프가 요구한 그림)
    #   early-repeat 이른 발 여러 번 → 되살리기 여러 번 → 성공
    #                ('한 번만 회복한다' 는 구현은 여기서 떨어진다)
    #
    # serverLag 를 고르는 기준: **회복 주기가 기계마다 다르다.** 한 주기는
    # 발사 → 제출 응답 → 알림 닫기 → [예약하기] → 확인창이고, 로컬에서는
    # 약 350~400ms, windows-latest 러너에서는 약 165ms 였다. 그래서 lag 을
    # 크게 잡으면 빠른 기계에서 상한 6회를 다 쓰고도 서버 정각에 못 닿는다
    # (2026-09-17 CI 실측: lag 1.2s → 7발 전부 too_early, exhausted).
    # 그것은 제품이 옳게 멈춘 것이지 결함이 아니다. 시나리오를 상한 안에
    # 들어오도록 줄인다: 가장 느린 주기(400ms)로도 2회, 가장 빠른 주기
    # (165ms)로도 6회 안에 끝나는 구간이 0.5~0.7s 다.
    cases = [
        # 이른 발 최소 1회 -> 되살리기 -> 성공.
        ("early-once", 0.25, 0.2, 2, booking.R_OK),
        # 되살리기를 여러 번 거쳐야 서버 정각을 넘긴다(상한 6회 안).
        ("early-repeat", 0.6, 0.2, 3, booking.R_OK),
    ]
    results = {}
